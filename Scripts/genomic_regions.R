...
annotations = function() {
  ...
  annots = c("hg38_cpgs", "hg38_genes_1to5kb", "hg38_genes_promoters", "hg38_genes_cds", "hg38_genes_5UTRs", "hg38_genes_exons",
...
genomic_regions_dmr = function(annotations) {
  ...
  for (i in seq(1,length(dmr_files))) {
    ...
    ggplot(sample_list, aes(fill = .data[[fill_name]], y = `Nº of DMR`, x = Regions)) +
      ...
      theme_light() +
      theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "top",
      axis.text.x = element_text(angle = 45, hjust = 1, size = 12), legend.title = element_text(size = 12),
