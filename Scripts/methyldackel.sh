...
do
    if compgen -G "${2}_rep${i}_*bam" > /dev/null; then
        if compgen -G "${2}_rep${i}_library_sorted.bam" > /dev/null; then
            methyl="$(MethylDackel mbias -l CpGIslands_mod.bed hg38.fa ${2}_rep${i}_library_sorted.bam ${2}_rep${i} 2>&1)"
            condition="$(echo ${methyl} | sed -e 's/^.*://g')"
            MethylDackel extract --mergeContext -l CpGIslands_mod.bed hg38.fa ${condition} ${2}_rep${i}_library_sorted.bam
